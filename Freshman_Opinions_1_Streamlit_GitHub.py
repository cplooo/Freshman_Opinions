...
with st.expander("學習及生活費（書籍、住宿、交通、伙食等開銷）主要來源(多選):"):
    ...
    if 院_系 == '0':
        collections = [df_freshman, df_freshman_faculty, df_freshman_original]
        dataframes = [Frequency_Distribution(df, column_index, split_symbol=';', dropped_string='沒有工讀', sum_choice=1) for df in collections]  ## 'dataframes' list 中的各dataframe已經是按照次數高至低的項目順序排列
        ## 形成所有學系'項目'欄位的所有值
        # desired_order  = list(set([item for df in dataframes for item in df['項目'].tolist()]))
        # desired_order  = list(set([item for item in dataframes[0]['項目'].tolist()])) 
